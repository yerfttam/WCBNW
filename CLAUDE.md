...
```
v1.1.6 fix: correct nav anchor IDs for dynamic sections
```
...
- The newsletter/deal banner is **hidden on all 7 pages** as of v1.1.5 using `<div hidden>`
- To restore: search for `<div hidden>` before the newsletter `<section>` in each page and remove the `<div hidden>` wrapper and its closing `</div>`
